...
class Inference():
    def _run(self,weights,source,project,name="exp",imgsz=[1280,800],conf_thres=0.25,max_det=1000,device="",line_thicknes=3,hide_labels=False,hide_conf=False) -> str:
        ...
        """
        infer.run(weights=weights,source=source,project=project,name=name,imgsz=imgsz,conf_thres=conf_thres,max_det=max_det,device=device,line_thickness=line_thicknes,hide_labels=hide_labels,hide_conf=hide_conf)
        print(f"Result dir is {result_dir}")
    ...
    def infer(self,weights,source,project,name="exp",imgsz=[640,640],conf_thres=0.25,max_det=1000,device="",line_thicknes=3,hide_labels=False,hide_conf=False) -> str:
